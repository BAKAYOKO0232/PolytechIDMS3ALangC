...
/*
procédure partition (entree/sortie t_tab tableau,
entree entier premier,
entree entier dernier,
sortie entier indicePivot)
*/
...
void TriRapide(int Tab[], int gauche, int droite)
{
    ...
    if (gauche < droite)
    {
        partition(Tab, gauche, droite, pivot);
        TriRapide(Tab,gauche, pivot-1);
        TriRapide(Tab,pivot+1, droite);
    }
}
...
void partition(int Tab[], int premier, int dernier, int indicePivot)
{
    ...
    for(i=premier+1; i<dernier; i++)
    {
        ...
        {
            compteur = compteur +1;
            echange(Tab[i], Tab[premier]);
        }
    }
    echange(Tab[compteur], Tab[premier]);
    indicePivot = compteur;
}
